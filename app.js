...
const MAIL_TYPES=["掛號","限掛","平信","黑貓宅急便","其他"];
let currentRole="",currentUser="系統使用者",currentUserEmail="";
let mailRecords=[],departmentList=[],userList=[],auditLogs=[],loginLogs=[];
...
function resetMailForm(){["mailId","trackingNo","sender","receiver","remark"].forEach(id=>$(id).value="");$("receiveDate").value=todayStr();$("mailType").value="掛號";$("department").value="";}
window.resetMailForm=resetMailForm;
async function saveMailRecord(){const id=$("mailId").value;const data={receiveDate:$("receiveDate").value,mailType:$("mailType").value,trackingNo:$("trackingNo").value.trim(),sender:$("sender").value.trim(),department:$("department").value,receiver:$("receiver").value.trim(),remark:$("remark").value.trim()};if(!data.receiveDate||!data.mailType||!data.sender||!data.department||!data.receiver){alert("請填寫收件日期、郵件類型、寄件人、收件部門、收件人");return}if(id){const before=mailRecords.find(r=>r.id===id);await updateDoc(doc(db,"mailRecords",id),data);await writeAuditLog({action:"update",category:"mailRecord",targetId:id,targetLabel:mailLabel(data),before,after:{...before,...data}});alert("修改成功")}else{const ref=await addDoc(collection(db,"mailRecords"),{...data,printed:false,createdBy:currentUser,createdByEmail:currentUserEmail,createdTime:new Date()});await writeAuditLog({action:"create",category:"mailRecord",targetId:ref.id,targetLabel:mailLabel(data),after:{...data,printed:false}});alert("登錄成功")}resetMailForm();await loadMailRecords();}
window.saveMailRecord=saveMailRecord;
function editMail(id){const r=mailRecords.find(x=>x.id===id);if(!r)return;showPage("registerPage",document.querySelector('[onclick*=registerPage]'));$("mailId").value=id;$("receiveDate").value=r.receiveDate||todayStr();$("mailType").value=r.mailType||"掛號";$("trackingNo").value=r.trackingNo||"";$("sender").value=r.sender||"";$("department").value=r.department||"";$("receiver").value=r.receiver||"";$("remark").value=r.remark||"";}
...
function renderMailTable(){const table=$("mailTable");if(!table)return;const filtered=getFilteredRecords();$("recordCount").textContent=filtered.length;const total=Math.ceil(filtered.length/pageSize)||1;if(currentPage>total)currentPage=1;const pageRows=filtered.slice((currentPage-1)*pageSize,currentPage*pageSize);table.innerHTML=pageRows.length?pageRows.map(r=>`<tr><td>${esc(formatDate(r.receiveDate))}</td><td>${esc(r.mailType)}</td><td>${esc(r.trackingNo||"-")}</td><td>${esc(r.sender)}</td><td>${esc(r.department)}</td><td>${esc(r.receiver)}</td><td>${r.printed?'<span class="badge badge-green">已列印</span>':'<span class="badge badge-yellow">未列印</span>'}</td><td><button class="btn btn-gray" onclick="editMail('${r.id}')">編輯</button> <button class="btn btn-gray" onclick="togglePrinted('${r.id}',${!!r.printed})">${r.printed?'改未列印':'標記列印'}</button> <button class="btn btn-red" onclick="deleteMail('${r.id}')">刪除</button></td></tr>`).join(""):`<tr><td colspan="8">目前沒有符合條件的郵件資料</td></tr>`;renderPagination(total)}
...
function exportExcel(){const data=getFilteredRecords().map(r=>({收件日期:formatDate(r.receiveDate),郵件類型:r.mailType,掛號單號:r.trackingNo||"",寄件人:r.sender,收件部門:r.department,收件人:r.receiver,備註:r.remark||"",列印狀態:r.printed?"已列印":"未列印",登錄人:r.createdBy||""}));const ws=XLSX.utils.json_to_sheet(data);const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,"郵件簽收紀錄");XLSX.writeFile(wb,"郵件簽收紀錄.xlsx")} window.exportExcel=exportExcel;
...
function actionLabel(a){return {create:"新增",update:"修改",delete:"刪除",print:"列印",permission:"權限異動",login:"登入"}[a]||a||"-"}function categoryLabel(c){return {mailRecord:"郵件資料",department:"部門",user:"使用者權限",signSheet:"簽收單"}[c]||c||"-"}function auditSummary(l){const o=l.after||l.before||{};return Object.entries(o).filter(([k])=>!["createdTime","printedAt"].includes(k)).map(([k,v])=>`${k}:${v}`).join("；")||"-"}
...
onAuthStateChanged(auth,async(user)=>{if(!user){$("loginPage").style.display="flex";$("systemArea").style.display="none";return}const snap=await getDocs(collection(db,"users"));let allow=false,found=null;snap.forEach(s=>{const d=s.data();if(d.email===user.email&&d.enabled===true){allow=true;found=d}});if(!allow){alert("此帳號未開通");await signOut(auth);return}currentRole=found.role||"user";currentUser=user.displayName||user.email;currentUserEmail=user.email;localStorage.setItem("userRole",currentRole);localStorage.setItem("userEmail",user.email);localStorage.setItem("userName",currentUser);if(!sessionStorage.getItem("mailLoginLogged")){await addDoc(collection(db,"mailLoginLogs"),{name:currentUser,email:user.email,role:currentRole,loginTime:new Date()});sessionStorage.setItem("mailLoginLogged","true")}$("sidebarUserName").textContent=currentUser;$("sidebarUserEmail").textContent=user.email;$("loginPage").style.display="none";try{await loadDepartments();await Promise.all([loadUsers(),loadMailRecords(),loadLoginLogs()]);}catch(error){console.error("系統初始化部分資料載入失敗",error);alert("系統資料載入失敗：" + (error?.message || error));}applyRoleAccess();restoreLastPage();$("systemArea").style.display="block";requestAnimationFrame(()=>$("systemArea").style.opacity="1");resetMailForm();if(window.lucide)lucide.createIcons();});
})();
