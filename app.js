...
const selectedReprintRecordIds=new Set();
...
async function loadMailRecords(){const snap=await getDocs(collection(db,"mailRecords"));mailRecords=[];snap.forEach(s=>mailRecords.push({id:s.id,...s.data()}));mailRecords.sort((a,b)=>(getTime(b.createdTime)||Date.parse(b.receiveDate||0))-(getTime(a.createdTime)||Date.parse(a.receiveDate||0)));selectedReprintRecordIds.forEach(id=>{if(!mailRecords.some(record=>record.id===id&&record.printed))selectedReprintRecordIds.delete(id)});renderDashboard();renderMailTable();renderSheetPage();refreshAllIncomingReceiverSuggestions()}
...
function incomingRowHtml(item={}, index=0){
  ...
  return `<tr class="outgoing-entry-row incoming-entry-row" data-id="${esc(item.id||'')}">
    ...
    <td class="incoming-row-actions">${item.id?'<span class="editing-row-note">編輯既有資料</span>':'<button type="button" class="row-action-btn" onclick="copyPreviousIncomingRow(this)">複製上一列</button><button type="button" class="row-action-btn" onclick="addBlankIncomingRow(this)">新增空白列</button><button type="button" class="row-action-btn row-action-danger" onclick="removeIncomingRow(this)">刪除本列</button>'}</td>
  </tr>`;
}
...
function renderMailTable(){const table=$("mailTable");if(!table)return;const outgoing=$("directionFilter")?.value==="outgoing";syncHistoryFilterMode(outgoing);const filtered=outgoing?getFilteredOutgoingRecords():getFilteredRecords();$("recordCount").textContent=filtered.length;const total=Math.ceil(filtered.length/pageSize)||1;if(currentPage>total)currentPage=1;const pageRows=filtered.slice((currentPage-1)*pageSize,currentPage*pageSize);const head=table.closest("table").querySelector("thead");if(outgoing){head.innerHTML="<tr><th>交寄日期</th><th>類型</th><th>掛號號碼</th><th>收件人</th><th>寄達地址</th><th>寄件人</th><th>批次</th><th>操作</th></tr>";table.innerHTML=pageRows.length?pageRows.map(r=>`<tr><td>${esc(formatDate(r.sendDate))}</td><td>${esc(r.mailType)}</td><td>${esc(r.trackingNo||"-")}</td><td>${esc(r.receiverName)}</td><td class="outgoing-address-cell">${esc(r.address)}</td><td>${esc(r.senderName||"-")}</td><td>${esc(r.batchNo)}</td><td><button class="btn btn-gray" onclick="editOutgoingBatch('${r.batchId}')">開啟批次</button></td></tr>`).join(""):`<tr><td colspan="8">目前沒有符合條件的寄件資料</td></tr>`}else{const selectableIds=pageRows.filter(r=>r.printed).map(r=>r.id),allSelected=selectableIds.length&&selectableIds.every(id=>selectedReprintRecordIds.has(id));head.innerHTML=`<tr><th class="reprint-check-col"><input type="checkbox" aria-label="選取本頁已列印郵件" ${allSelected?'checked':''} onchange="togglePageReprintSelection(this.checked)"></th><th>收件日期</th><th>類型</th><th>掛號 / 單號</th><th>寄件人</th><th>部門</th><th>收件人</th><th>狀態</th><th>操作</th></tr>`;table.innerHTML=pageRows.length?pageRows.map(r=>`<tr><td class="reprint-check-col">${r.printed?`<input type="checkbox" aria-label="選取補印 ${esc(mailLabel(r))}" ${selectedReprintRecordIds.has(r.id)?'checked':''} onchange="toggleReprintSelection('${r.id}',this.checked)">`:'—'}</td><td>${esc(formatDate(r.receiveDate))}</td><td>${esc(r.mailType)}</td><td>${esc(r.trackingNo||"-")}</td><td>${esc(r.sender)}</td><td>${esc(r.department)}</td><td>${esc(r.receiver)}</td><td>${r.printed?'<span class="badge badge-green">已列印</span>':'<span class="badge badge-yellow">未列印</span>'}</td><td><button class="btn btn-gray" onclick="editMail('${r.id}')">編輯</button> ${r.printed?`<button class="btn btn-primary" onclick="reprintMail('${r.id}')">補印</button>`:`<button class="btn btn-gray" onclick="togglePrinted('${r.id}',false)">標記列印</button>`} <button class="btn btn-red" onclick="deleteMail('${r.id}')">刪除</button></td></tr>`).join(""):`<tr><td colspan="9">目前沒有符合條件的郵件資料</td></tr>`}updateReprintActions(outgoing);renderPagination(total)}
...
function currentIncomingHistoryPageRows(){const filtered=getFilteredRecords();return filtered.slice((currentPage-1)*pageSize,currentPage*pageSize)}
function updateReprintActions(outgoing=$("directionFilter")?.value==="outgoing"){const area=$("reprintActions");if(!area)return;area.hidden=outgoing;$("reprintSelectionCount").textContent=`已選 ${selectedReprintRecordIds.size} 筆`;$("reprintSelectedButton").disabled=selectedReprintRecordIds.size===0}
function toggleReprintSelection(id,checked){if(checked)selectedReprintRecordIds.add(id);else selectedReprintRecordIds.delete(id);renderMailTable()} window.toggleReprintSelection=toggleReprintSelection;
function togglePageReprintSelection(checked){currentIncomingHistoryPageRows().filter(r=>r.printed).forEach(r=>checked?selectedReprintRecordIds.add(r.id):selectedReprintRecordIds.delete(r.id));renderMailTable()} window.togglePageReprintSelection=togglePageReprintSelection;
function clearReprintSelection(){selectedReprintRecordIds.clear();renderMailTable()} window.clearReprintSelection=clearReprintSelection;
...
function reprintMail(id){const record=mailRecords.find(r=>r.id===id&&r.printed);if(!record){alert("找不到已列印郵件");return}prepareReprint([record],{batchNo:record.printBatchNo||"",label:`單筆補印：${mailLabel(record)}`})} window.reprintMail=reprintMail;
function reprintSelectedRecords(){const records=[...selectedReprintRecordIds].map(id=>mailRecords.find(r=>r.id===id&&r.printed)).filter(Boolean);prepareReprint(records,{label:`選取補印，共 ${records.length} 筆`})} window.reprintSelectedRecords=reprintSelectedRecords;

function renderSheetPage(){if(!$("sheetDeptList"))return;if(!$("sheetDate").value)$("sheetDate").value=todayStr();const date=$("sheetDate").value;const only=$("sheetOnlyUnprinted").checked;const rows=mailRecords.filter(r=>(!date||r.receiveDate===date)&&(!only||!r.printed));const grouped=groupCount(rows,"department");$("sheetDeptList").innerHTML=grouped.length?grouped.map(([dept,count])=>`<label class="dept-check-card"><div><strong>${esc(dept)}</strong><br><span>${count} 件郵件</span></div><input type="checkbox" class="sheet-dept-check" value="${esc(dept)}"></label>`).join(""):`<div class="summary-item"><strong>沒有可產生簽收單的郵件</strong><span>0 件</span></div>`;renderPrintBatchHistory()}
...
function actionLabel(a){return {create:"新增",update:"修改",delete:"刪除",print:"列印",reprint:"補印",permission:"權限異動",login:"登入"}[a]||a||"-"}function categoryLabel(c){return {mailRecord:"郵件資料",outgoingMailBatch:"大宗寄件",department:"部門",mailType:"郵件類型",user:"使用者權限",signSheet:"簽收單"}[c]||c||"-"}function auditSummary(l){const o=l.after||l.before||{};return Object.entries(o).filter(([k])=>!["createdTime","printedAt","items"].includes(k)).map(([k,v])=>`${k}:${v}`).join("；")||"-"}
