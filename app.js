...
function incomingRowHtml(item={}, index=0){
  ...
  return `<tr class="outgoing-entry-row incoming-entry-row" data-id="${esc(item.id||'')}">
    ...
    <td class="incoming-row-actions">${item.id?'<span class="editing-row-note">編輯既有資料</span>':'<div class="row-action-group"><button type="button" class="row-action-btn row-action-copy" title="複製上一列內容" onclick="copyPreviousIncomingRow(this)">複製</button><button type="button" class="row-action-btn row-action-add" title="在下方新增空白列" onclick="addBlankIncomingRow(this)">新增</button><button type="button" class="row-action-btn row-action-danger" title="刪除這一列" onclick="removeIncomingRow(this)">刪除</button></div>'}</td>
  </tr>`;
}
