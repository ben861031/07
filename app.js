...
function renderSheetPage(){if(!$("sheetDeptList"))return;if(!$("sheetDate").value)$("sheetDate").value=todayStr();const date=$("sheetDate").value,only=$("sheetOnlyUnprinted").checked,rows=mailRecords.filter(r=>(!date||r.receiveDate===date)&&(!only||!r.printed)),grouped=groupCount(rows,"department");$("sheetDeptList").innerHTML=grouped.length?grouped.map(([dept,count])=>`<label class="dept-check-card"><div><strong>${esc(dept)}</strong><br><span>${count} 件郵件</span></div><input type="checkbox" class="sheet-dept-check" value="${esc(dept)}" onchange="updateSheetGenerateButton()"></label>`).join(""):`<div class="sheet-empty-state"><i data-lucide="inbox"></i><div><strong>${only?'此日期沒有未列印郵件':'此日期沒有郵件資料'}</strong><span>${only?'可取消上方「只顯示未列印郵件」查看已列印資料。':'請更換收件日期後再試一次。'}</span></div><b>0 件</b></div>`;updateSheetGenerateButton();clearSheetPreview();renderPrintBatchHistory();if(window.lucide)lucide.createIcons()}
